# 🛡️ CyberDost v3.0 — Grand Finale Build

**India's First Bilingual AI Cyber-Safety Companion**

> KRIWA National AI-Coding Challenge 2026 | Theme 9: Cyber-Safe Portal
> Team KRIWA-26-1102 | Tathagata Laskar (Lead) & Devaansh Singh
> Chandigarh University

## Features
- **120+ Threat Patterns** — KYC fraud, digital arrest, lottery, loan, job scams, SIM swap, electricity bill scam, investment fraud, AnyDesk/TeamViewer remote access
- **60+ Bank Sender IDs** — SBI, HDFC, ICICI, Axis, BOB, Union Bank, PNB, Kotak, and more
- **Bilingual** — Full Hindi (Devanagari) and English with real-time toggle
- **Secure Auth** — Login with password strength enforcement
- **Analysis History** — Track all past scans
- **Real Bank SMS Recognition** — Tested with actual BOB and Union Bank messages
- **Z-Plus Security** — Source maps disabled, ownership watermarks, base64 fingerprints

## Tech Stack
React 18 | Groq AI (Llama 3.3) | CSS-in-JS | Vercel

## Setup
```bash
npm install && npm start
```

## Deploy
```bash
npm run build  # → deploy build/ to Vercel
```

## License
Proprietary — © 2026 Tathagata Laskar & Devaansh Singh
